	...
	if(lflag > 0){
     ...
     //debug out
  	if (dflag == 1 || dflag == 2){
   	 ...
   	 fprintf(debug_output, "Entering function %s (MDptr=%p)\n",
		    __func__, (void*)MDp);
  	}
  	if(lflag > 0){
  		fclose(debug_output);
	...
	if(lflag > 0){
     ...
     //debug out
  	if (dflag == 1 || dflag == 2){
   	 ...
   	 fprintf(debug_output, "Entering function %s (MDptr=%p, X=%lu)\n",
		    __func__, (void*)MDp, (long unsigned int)X);
  	}
  	if(lflag > 0){
  		fclose(debug_output);
  	}
...
void MDupdate(MDptr MDp,unsigned char *X,unsigned int count){
	...
	if(lflag > 0){
     ...
     //debug out
  	if (dflag == 1 || dflag == 2){
   	 ...
   	 fprintf(debug_output, "Entering function %s (MDptr=%p, X=%s, count=%u)\n",
		    __func__, (void*)MDp, X,count);
  	}
  	if(lflag > 0){
  		fclose(debug_output);
  	}
	...
	if(count == 512){
		...
		printf("\nError: MDupdate called with illegal count value %ld.",(long int)count);
		return;
	} else {
