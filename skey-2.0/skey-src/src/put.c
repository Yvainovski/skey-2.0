...
char *btoe (char *engout, char *c){
  ...
  if(lflag > 0){
    ...
    if (dflag == 1 || dflag == 2){
     ...
     fprintf(debug_output, "Entering function %s (engout=%s, c=%s)\n",
        __func__, engout, c);
    }
    if(lflag > 0){
      fclose(debug_output);
...
static unsigned long extract (char *s, int start, int length){
  ...
  if(lflag > 0){
    ...
    if (dflag == 1 || dflag == 2){
     ...
     fprintf(debug_output, "Entering function %s (char=%s, start=%d, length=%d)\n",
        __func__, s,start,length);
    }
    if(lflag > 0){
      fclose(debug_output);
    }
  ...
  if(lflag > 0){
    ...
    if (dflag == 1){
      ...
      fprintf(debug_output, "Exiting function %s (ret=%lu)\n", __func__, x);
    }
    //close the file
    if(lflag > 0){
      fclose(debug_output);
